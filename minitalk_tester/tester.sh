...
if [[ $t4 == 1 || $tm == 1 ]]
then
    echo "${L_Green}\n [Test 4]\tTrying to crash your exchange Server-Client, sending str with 3 000 char (15 times) ${NC}"
...
if [[ $t6 == 1 || $tb == 1 ]]
then
    echo "${L_Green}\n [Test 6]\tTesting the connection between server-client, sending str with 5 300 char (10 times)${NC}"
    for i in {1..10}
    do
        echo -n "${L_Green} [ ${i} ]${NC}"
        $PATH_TO_CLIENT $p_id "
...
echo "\nIf all was correctly display on your server terminal: Perfect, well done! 💪"
